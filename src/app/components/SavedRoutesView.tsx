...
import { Eye, EyeOff, Trash2, MapPin, Clock, Ruler, Phone, User, ChevronDown, ChevronUp, FileDown } from 'lucide-react';
...
export function SavedRoutesView({ savedRoutes, onDeleteRoute, onToggleVisibility }: SavedRoutesViewProps) {
  ...
  const renderSavedRoutes = () => {
    ...
    const routesToShow = savedRoutes.filter(route => route.routeType === activeTab);
    ...
    routesToShow.forEach((route) => {
      ...
      passengerDataList.forEach(passenger => {
        ...
        if (passenger.destinationCoordinates) {
          ...
          destEl.style.width = '26px';
          destEl.style.height = '26px';
    ...
    if (routesToShow.some(r => r.visible)) {
      ...
      routesToShow.forEach(route => {
        ...
        passengerDataList.forEach(passenger => {
          ...
          if (passenger.destinationCoordinates && passenger.destinationCoordinates.length === 2) {
  ...
  const goingRoutes = savedRoutes.filter(route => route.routeType === 'going');
  const returnRoutes = savedRoutes.filter(route => route.routeType === 'return');
  ...
  const exportToExcel = (routeType: 'going' | 'return') => {
    const routesToExport = savedRoutes.filter(route => route.routeType === routeType);
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, `${routeType.charAt(0).toUpperCase() + routeType.slice(1)} Routes`);
    ...
    routes.length === 0 ? (
      <Card>
        <CardContent className="py-12 text-center">
          ...
          <p className="text-sm text-gray-500">Create your first route to get started</p>
        </CardContent>
      </Card>
    ) : (
          ...
          return (
            ...
            >
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <p className="text-xs text-blue-600 font-medium">
                          {route.routeInfo.distance.toFixed(1)} km • {Math.round(route.routeInfo.duration)} min
                        </p>
                      </div>
                    </div>
                    <div className="ml-2">
                  ...
                  <div className="flex items-center gap-1 ml-2">
                    ...
                    >
                      {route.visible ? <Eye className="w-4 h-4 text-blue-600" /> : <EyeOff className="w-4 h-4 text-gray-400" />}
                    </Button>
                    <Button
                      size="sm"
                      variant="ghost"
              ...
              {isExpanded && (
                ...
                <div>
                  <p className="text-xs font-semibold text-gray-700 mb-2">Driver</p>
                  {driverData ? (
                  ...
                  <div className="space-y-2">
                    {passengerDataList.length > 0 ? (
                      passengerDataList.map((passenger) => (
                        <div key={passenger.id} className="bg-green-50 rounded-lg p-3 space-y-2">
                          <div className="flex items-center gap-2">
                            <User className="w-4 h-4 text-green-600" />
                            <span className="text-sm font-medium text-gray-900">{passenger.name}</span>
                          </div>
                          <div className="flex items-center gap-2">
                            <Phone className="w-4 h-4 text-green-600" />
                            <span className="text-sm text-gray-700">{passenger.phone}</span>
                          </div>
                        </div>
                      ))
                    ) : (
                      <p className="text-xs text-gray-500">No passengers found</p>
                    )}
                  </div>
                </div>
                ...
                <div className="pt-2 border-t">
                  <div className="flex items-center gap-2">
                    ...
                    />
                    <span className="text-xs text-gray-600">Route Color: {route.color.name}</span>
                  </div>
                </div>
              </CardContent>
              )}
    ...
    <div className="h-full flex overflow-hidden">
      <div className="w-96 bg-white border-r overflow-y-auto">
        <div className="p-4 space-y-4">
          ...
          <Tabs defaultValue="going" className="w-full" onValueChange={(value) => setActiveTab(value as 'going' | 'return')}>
            <div className="flex items-center justify-between gap-2 mb-3">
              ...
              <Button
                ...
                variant="outline"
                className="gap-2"
                disabled={activeTab === 'going' ? goingRoutes.length === 0 : returnRoutes.length === 0}
              >
        ...
        {savedRoutes.length > 0 && (
          <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white px-6 py-3 rounded-full shadow-lg z-10">
            <p className="text-sm text-gray-600">
              Showing {savedRoutes.filter(r => r.visible).length} of {savedRoutes.length} routes
            </p>
          </div>
        )}
      </div>
